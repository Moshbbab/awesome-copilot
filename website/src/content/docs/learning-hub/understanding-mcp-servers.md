...
- **Client ID Metadata Document (CIMD)** *(v1.0.83+)*: MCP OAuth sign-in supports CIMD, which lets a server resolve client metadata (name, redirect URIs, etc.) from a hosted document referenced by the client ID URL instead of requiring manual client registration.
