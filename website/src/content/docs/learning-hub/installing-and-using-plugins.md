...
> **Dedicated CLI commands (v1.0.84+)**: `copilot instruction list` and `copilot lsp list` replace the cross-kind `copilot plugins list --kind instruction`/`--kind lsp` flags, and `enable`/`disable` subcommands are now available directly on `copilot plugin`, `copilot mcp`, and `copilot skill` (e.g. `copilot skill disable <name>`), replacing `copilot plugins enable/disable --plugin|--mcp|--skill`. The generic `--kind`, `--scope`, `--mcp`, and `--skill` flags have been removed from `copilot plugins` entirely — use the kind-specific commands (`copilot mcp`, `copilot skill`, `copilot instruction`, `copilot lsp`) instead. `copilot plugins install --skill [--scope project]` is replaced by `copilot skill add [--project]`, and `copilot plugins list` is now a plugins-only alias of `copilot plugin list` (it no longer reports MCP servers, skills, instructions, or LSP servers, and its `--json` output is a flat array instead of the old `{ plugins, errors }` object).
