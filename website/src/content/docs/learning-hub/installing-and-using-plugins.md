...
> **Breaking change — cross-kind commands retired (v1.0.84+)**: The old cross-kind `copilot plugins list --kind instruction|lsp` flags are gone. Use the dedicated `copilot instruction list` and `copilot lsp list` commands instead. Similarly, `copilot plugin`, `copilot mcp`, and `copilot skill` each gained their own `enable`/`disable` subcommands, replacing `copilot plugins enable/disable --plugin|--mcp|--skill`. `copilot plugins install --skill [--scope project]` is replaced by `copilot skill add [--project]` (the `--scope` spelling is gone). `copilot plugins list` is now a strict alias of `copilot plugin list` — it reports only plugins (no MCP servers, skills, instructions, or LSP servers), and with `--json` it emits a flat array instead of the old `{ plugins, errors }` object, so scripts reading `.plugins` need updating. `copilot plugin list`, `copilot plugin marketplace list`, and `copilot plugin marketplace browse` all gained a `--json` flag for scripting.
