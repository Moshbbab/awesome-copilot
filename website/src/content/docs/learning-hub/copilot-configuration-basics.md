...
**Vim mode for everyone** *(v1.0.84+)*: Vim-style modal editing in the composer is now available to all users. Turn it on with `/vim`, or set `editorMode` to `"vim"` in your config file. The current mode (normal/insert) is shown while you type, so you always know which mode is active.
