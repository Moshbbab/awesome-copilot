...
> **Vim mode generally available (v1.0.84+)**: Vim-style modal editing in the composer is now available to everyone (previously experimental/opt-in). Turn it on with the `/vim` slash command or by setting `editorMode` to `vim`; the current mode (insert/normal) is shown while you type.
