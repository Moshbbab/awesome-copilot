...
*(v1.0.84+)* Vim mode is available to everyone in the composer. Turn it on with `/vim`, or set `editorMode` to `vim` in your settings for modal editing (normal, insert, and visual modes) when composing prompts. The current mode is shown while you type, so you always know whether you're in normal or insert mode.

### Plugin Component Listing Commands

*(v1.0.84+)* `copilot instruction list` and `copilot lsp list` replace the older `copilot plugins list --kind instruction` and `--kind lsp` cross-kind flags, giving each plugin component type its own dedicated listing command. Similarly, `enable` and `disable` subcommands are now available directly on `copilot plugin`, `copilot mcp`, and `copilot skill` (for example, `copilot mcp disable <name>`), replacing the old `copilot plugins enable/disable --plugin|--mcp|--skill` flag-based syntax.
