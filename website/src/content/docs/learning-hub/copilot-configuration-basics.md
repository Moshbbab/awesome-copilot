...
| `editorMode` | Set to `vim` for modal (vim-style) editing in the composer, with the current mode shown while you type (v1.0.84+) |
...
> **Vim mode (v1.0.84+)**: Turn on modal, vim-style editing in the composer with `/vim`, or set `editorMode` to `vim` in your config. The current mode (normal/insert) is shown while you type, so you always know which mode you're in.
