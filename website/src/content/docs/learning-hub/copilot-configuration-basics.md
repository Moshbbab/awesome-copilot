...
| `editorMode` | Set to `vim` to enable Vim keybindings (modal editing) in the composer; can also be toggled per session with `/vim` (v1.0.84+) |
| `taskbarPresence` | Set to `false` to disable the Windows 11 taskbar session status indicator (v1.0.84+) |
...
**Windows 11 taskbar status** *(v1.0.83+)*: On Windows 11, running Copilot CLI sessions now appear in the taskbar with live hover status cards, so you can check on a background session's progress without switching back to its terminal window. Use `/settings taskbarPresence false` to disable this if you'd rather not have sessions show up in the taskbar.

**Vim mode** *(v1.0.84+)*: Vim keybindings for the composer are now available to everyone. Turn it on for the current session with `/vim`, or set `editorMode` to `vim` in `/settings` to enable modal editing by default. The current mode (normal or insert) is shown while you type.
...
**Enterprise network and auth additions** *(v1.0.83+)*: Enterprise admins can pin sign-in to a set of approved GitHub organizations with the managed `forceLoginOrgs` setting. The CLI's `proxy` setting also gains automatic HTTPS proxy **mTLS client certificate** support for model and web requests, and Kerberos-authenticated proxies now reconnect correctly when the initial challenge responds with `Connection: close`. These are managed/enterprise-scoped settings, typically configured centrally rather than per user.
